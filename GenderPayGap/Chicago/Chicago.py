...
print("Number of different Job Titles (total): ", df['Job Titles'].nunique())

# Removing Job Titles with less than 100 occurrences
df = df.groupby('Job Titles').filter(lambda o: len(o) > 100)

# Removing Part-Time employees (as suggested in the Glassdoor report)
#df = df[df['Full or Part-Time'] == 'F']

print("\n# Filtered DataFrame (more than 100 Job Titles occurrences) #"
      "\nMales: ", len(df[df['Gender'] == 'male']), "\t\tFemales: ", len(df[df['Gender'] == 'female']),
      "\nNumber of different Job Titles: ", df['Job Titles'].nunique(), "\n")
#print(df.groupby(['Job Titles', 'Gender']).size(), "\n")
...
print(df, "\n\nMales: ", len(df[df['Gender'] == 'male']), "\t\tFemales: ", len(df[df['Gender'] == 'female']),
      "\nNumber of different Job Titles (total): ", df['Job Titles'].nunique(), "\n")
...
print("\n", pd.pivot_table(df, index=["Job Titles", "Gender"], values=["Annual Salary"], aggfunc=[np.average, len]))

# Run Your Regressions #
t = PrettyTable()
t.add_column('', ['Male', 'Job Titles', 'Department', 'Full or Part-Time', 'Constant',
                  'Controls\n   - Job Titles\n   - Department\n   - Full or Part-Time', 'Observations', 'R^2'])
...
# Add controls for Job Titles, Department and Full or Part-Time ("adjusted" pay gap)
x = df[['Male', 'Job Titles', 'Department', 'Full or Part-Time']]
...
dfRF['Job Titles'] = label_encoder.fit_transform(dfRF['Job Titles'])
...
# Exporting CSV
dfRF.to_csv(r'Chicago_RankingFacts.csv', index=False)
quit()
'''
...
bins = [0, 39.99, 59.99, 79.99, 99.99, 119.99, 139.99, 200]
#labels = ["< 90K", "≥ 90K"]
#bins = [0, 89.99, 200]
...
# Removing useless columns
del df['Name'], df['Salary or Hourly'], df['Annual Salary'], df['Log Annual Salary'], df['Male']
...
# Exporting CSV
#df.to_csv(r'Chicago_FAIR-DB.csv', index=False, line_terminator=',\n')
#df.to_csv(r'Chicago_FAIR-DB_2bins.csv', index=False, line_terminator=',\n')
...
with open('cfds[s=100_FPT].txt') as f:
...
#indexArray = [0, 7, 8, 10, 13] with support 2100
